...
def create_twitter_card(obj):
    ...
    obj must implement:
      ...
      - obj.get_card_name()  → str
      - obj.get_card_image() → relative path like "db/slug.png"
    """
    ...
    if not obj.logo:
        is_text = True
        name = obj.get_card_name()
        lines = name.split() if " " in name else [name]
        display_name = "\n".join(lines)

        # Shrink font until the widest line fits within the right panel
        max_text_width = new_im.width - settings.OG_CARD_BASE_OFFSET_X - 2 * settings.OG_CARD_MARGIN
        font_size = 512
        while font_size > 12:
    ...
    # SVG logo — rasterize at the panel dimensions so PIL only ever scales down
        ...
        panel_w = new_im.width - settings.OG_CARD_BASE_OFFSET_X - 2 * settings.OG_CARD_MARGIN
        panel_h = new_im.height - 2 * settings.OG_CARD_MARGIN
    ...
    if not is_text:
        max_w = new_im.width - settings.OG_CARD_BASE_OFFSET_X - 2 * settings.OG_CARD_MARGIN
        max_h = new_im.height - 2 * settings.OG_CARD_MARGIN
    ...
    # Center the logo in the right panel (which starts at OG_CARD_BASE_OFFSET_X)
    content_width = new_im.width - settings.OG_CARD_BASE_OFFSET_X
    offset = (
        settings.OG_CARD_BASE_OFFSET_X + (content_width - logo.width) // 2,
        (new_im.height - logo.height) // 2,
    )

    card_rel = obj.get_card_image()
    card_img = os.path.join(settings.OG_CARD_ROOT, card_rel)
